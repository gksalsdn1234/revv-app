...
### 코드가 아닌 것 — 온보딩 (출시 전에 정해야 함)

이번 분석에서 가장 값싼 기회는 기능이 아니라 **저쪽이 무너진 자리**다. 근거 둘:

- 스토어 리뷰 `[확인]`: 루트를 다 짜게 만든 뒤 계정을 요구하고, 다시 짜게 한 다음
  페이월. "skip for now"가 없어 체험 자체가 불가능하다는 불만.
- 분석 이벤트 `[확인]`: **`Free Roam Abandoned During Voice Pack Wait`** —
  보이스팩 다운로드 대기 중 이탈을 **별도 이벤트로 센다.**

**해석 `[추론]`**: 저쪽은 첫 가치를 주기 전에 **결제와 대기**를 요구하고, 그게 실제
이탈 지점이라는 것을 스스로 알고 있다(그래서 이벤트를 만들었다).

**→ REVV의 방향: 첫 루트는 조건 없이 준다.** 계정·결제·다운로드 대기를 첫 성공
경험 뒤로 민다. 우리는 루트 83,232개를 이미 갖고 있어서 "즉시 뭔가를 보여주는 것"이
저쪽보다 훨씬 쉽다 — 저쪽은 사용자가 목적지를 정해줘야 시작된다.
이건 코드 작업이 아니라 **출시 전에 확정해야 하는 설계 결정**이다.

### 하지 말 것
- 저쪽 mp3·카피·알고리즘 상수 사용 (저작권 + clean-room 오염)
- 루트 발견 축을 버리고 페이스노트 앱으로 전향하는 것 — **저쪽이 못 가진 게 그거다**
- GPX/KML 임포트를 저쪽만큼 우선시하는 것 — 저쪽은 루트를 못 찾아주니 남의 루트를
  끌어와야 하는 것이고, 우리는 그 문제가 없다. 유입 경로로는 유효하나 우선순위는 낮다.
