...
ADD COLUMN IF NOT EXISTS road_names JSONB DEFAULT '[]'::jsonb,
ADD COLUMN IF NOT EXISTS surface_summary TEXT DEFAULT '',
ADD COLUMN IF NOT EXISTS speed_limit_summary TEXT DEFAULT '',
ADD COLUMN IF NOT EXISTS nearby_pois JSONB DEFAULT '[]'::jsonb,
ADD COLUMN IF NOT EXISTS route_context JSONB DEFAULT '{}'::jsonb,
ADD COLUMN IF NOT EXISTS context_version TEXT DEFAULT '',
ADD COLUMN IF NOT EXISTS context_enriched_at TIMESTAMPTZ;

COMMENT ON COLUMN curvy_roads.elevation_profile IS
  'Optional sampled elevation profile in meters, aligned to route order when available.';
COMMENT ON COLUMN curvy_roads.road_names IS
  'Most relevant OSM road names intersecting the route corridor.';
COMMENT ON COLUMN curvy_roads.surface_summary IS
  'Dominant OSM surface values near the route corridor.';
COMMENT ON COLUMN curvy_roads.speed_limit_summary IS
  'Dominant OSM maxspeed values near the route corridor.';
COMMENT ON COLUMN curvy_roads.nearby_pois IS
  'Compact nearby POI list used for route explanation context.';
COMMENT ON COLUMN curvy_roads.route_context IS
  'Additional route context such as dominant highway classes, refs, surfaces, and POIs.';
COMMENT ON COLUMN curvy_roads.context_version IS
  'Version tag for route-context enrichment.';
COMMENT ON COLUMN curvy_roads.context_enriched_at IS
  'Timestamp when route-context enrichment was last computed.';

CREATE INDEX IF NOT EXISTS idx_curvy_roads_context_version
  ON curvy_roads(context_version);
